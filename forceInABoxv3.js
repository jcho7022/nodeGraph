...
d3.layout.forceInABox = function () {
  ...
  force.
  StrengthInterCluster = function (x) {
    ...
    templateNodes.forEach(function (d) {
      if (template==="treemap") {
        ...
        boxBound[d.id] = {
          ...
          lx : (d.x-40),  //TODO
          ly : (d.y),
          centroidx: (d.x + d.dx / 2),
          centroidy:(d.y + d.dy / 2)
        };
  ...
  function drawTreemap(container) {
    ...
    container.selectAll("cell")
      ...
      .attr("class", "cell")
      .attr("x", function (d) { return d.x -40; }) //TODO
  ...
  force.onTick = function (e) {
    ...
    force.nodes().forEach(function (o) {
      ...
      if(groupInitialFlag){
        setTimeout(function(){
          if(groupInitialFlag){
            groupInitialFlag = false;
            force.stop();
            o.x = Math.max(boxBound[groupBy(o)].lx+30, Math.min(boxBound[groupBy(o)].hx -30, o.x));
            o.y = Math.max(boxBound[groupBy(o)].ly+30, Math.min(boxBound[groupBy(o)].hy -30, o.y));
            force.start();
          }
        }, 1000);
      }
